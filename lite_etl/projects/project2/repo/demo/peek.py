...
def show_inputs() -> None:
    ...
    print("  fx_rates         : USD/EUR/GBP per month; GBP missing for current month -> carry forward")
...
def show_state() -> None:
    ...
    _rule("MRR by month  (from subscriptions x plan price)")
    for m, tot in con.execute(
        "select month, round(sum(mrr_usd),2) from mrr_by_month group by month order by month"
    ):
        print(f"  {m}   {tot:12,.2f}")
