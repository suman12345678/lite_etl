"""project2 end-to-end, local, zero-cloud.  See ../DEMO.md.
...
def main(argv: list[str] | None = None) -> int:
    ...
    for tbl in ("stg_invoices", "stg_accounts", "stg_subscriptions", "dim_account",
                "dim_plan", "fct_invoice", "fct_mrr_movement"):
        n = con.execute(f"select count(*) from {tbl}").fetchone()[0]
        print(f"    {tbl:<20} {n:>4} rows")
    deduped = con.execute("select count(*) from raw_invoices").fetchone()[0] - con.execute(
        "select count(*) from stg_invoices"
    ).fetchone()[0]
    merged = con.execute(
        "select count(*) from raw_accounts where merged_into is not null"
    ).fetchone()[0]
