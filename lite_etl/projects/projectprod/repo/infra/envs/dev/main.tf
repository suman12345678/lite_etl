terraform {
  ...
  #                      -backend-config="bucket=$TF_STATE_BUCKET" -backend-config="key=projectprod/dev.tfstate"`
}

variable "engine" {
  type    = string
  default = "duckdb" # dev runs the same models on DuckDB -- no cloud warehouse needed
}
