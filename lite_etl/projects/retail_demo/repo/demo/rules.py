...
reconcile -> block                          (aggregate agrees with finance)

Returns a dict the runner uses to decide publish vs block.
"""
...
def _load_control_totals(con: sqlite3.Connection, web_multiplier: float) -> None:
    """Finance's source-of-truth totals -> an in-memory table the reconcile SQL reads.
    web_multiplier != 1.0 simulates finance and the pipeline disagreeing."""
    con.execute("drop table if exists control_totals")
    con.execute("create table control_totals (channel text, expected_net_usd real)")
    with open(CONTROL_CSV, newline="") as fh:
        for row in csv.DictReader(fh):
            val = float(row["expected_net_usd"])
            if row["channel"] == "web":
                val *= web_multiplier
            con.execute("insert into control_totals values (?, ?)", (row["channel"], val))
...
def run(db_path: Path, web_control_multiplier: float = 1.0) -> dict:
    ...
    for rule in spec.get("quality", []):
        ...
        bad = con.execute(
            f"select count(*) from {table} where not ({expect})"
        ).fetchone()[0]
    ...
    # ---- reconcile -----------------------------------------------------------
    _load_control_totals(con, web_control_multiplier)
