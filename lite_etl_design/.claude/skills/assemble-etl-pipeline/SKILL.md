...
3. Read `<WS>/design/` (especially `deployment-and-iac.md`, `pipeline-blueprint.md`,
   `10-platform-and-deployment.md`, `architecture-overview.md`) and `<WS>/build/`
...
- **Repo model** - mono-repo vs split, host, from `10-platform-and-deployment.md`.
- **Orchestrator deployment** - image / code-location / bundle; how schedules and
  ...
  check / short-circuit task) from `pipeline-blueprint.md`.
...
- `infra/modules/<name>/{main.tf,variables.tf,outputs.tf}` and
  `infra/envs/{dev,test,prod}/{backend.tf,main.tf,<env>.tfvars}` - resource
  blocks named and wired, variables declared, **values as `var.` refs or `TODO`**.
- The orchestrator project (`dagster/` / `dags/` / `flows/`): one pipeline file
  per `pipeline-blueprint.md` pipeline with tasks named, dependency edges wired,
  retry config set, the gate as a short-circuit; `resources`/`config`,
  `schedules`, `sensors`, `backfill` stubs.
...
- `.pre-commit-config.yaml`, `CODEOWNERS`, `config/{defaults,dev,test,prod}.yml`
  skeletons.
...
Suggest running `/validate-config`.
